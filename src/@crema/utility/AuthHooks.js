...
import {getUserFromBank, getUserFromJwtAuth} from './helper/AuthHelper';
import {
  useJWTAuth,
  useJWTAuthActions,
...
export const useAuthUserBankData = () => {
  const {user, isAuthenticated, isLoading} = useJWTAuth();
  return {
    isLoading,
    isAuthenticated,
    bank: getUserFromBank(user),
  };
};

export const useAuthMethod = () => {
  const {signInUser, signUpUser, logout, updatePasword, forgetPassword} =
    useJWTAuthActions();
