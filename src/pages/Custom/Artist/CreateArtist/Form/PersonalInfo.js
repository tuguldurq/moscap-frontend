...
const PersonalInfo = (props) => {
  ...
  return (
    <>
      ...
      <Form.List name={['user', 'emergency-list']}>
        {(fields, {add, remove}) => (
          <>
            {fields.map((field) => (
              <div key={field.key}>
      ...
      {isUpdate && (
        <Form.Item label='Нууц үг солих'>
          <Switch onChange={(e) => setIsPasswordReset(e)} />
        </Form.Item>
      )}
      {(!isUpdate || isPasswordReset) && (
        <Row gutter={8}>
          <Col md={12} xs={24}>
            <Form.Item
              name={['user', 'password']}
